...
export async function getUserTokens(walletAddress: string) {
  ...
  tokens.push({
    ...
    amount: solBalance,
    decimals: 9,
    uiAmount: solBalance.toFixed(4), // Formatted for display
  });

  // Add SPL tokens (only non-zero balances)
  for (const { account } of tokenAccounts.value) {
    ...
    tokens.push({
      ...
      uiAmount: amount.toFixed(parsedInfo.tokenAmount.decimals), // Formatted
